...
for i in range(0,len(coords)):
    cityList.append(ga.City(node=i , x=coords[i][1] , y= coords[i][2] ))
...
if __name__ == "__main__":
    ...
    t1=time.time()
    cost=ga.geneticAlgorithm(cityList, popSize=int(params["popSize"]),eliteSize = int(params["eliteSize"]), mutationRate=float(params["mutationRate"]),generations = int(params["population"]))
    t2=time.time()
